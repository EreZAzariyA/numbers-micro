...
class Microservices {
  ...
  register(service: Service) {
    ...
    this.services[key].timestamp = new Date().valueOf();
    console.debug(`Updated services ${service.name}, version ${service.version} at ${service.ip}:${service.port}`);
    return key;
  };
  ...
  unregister(service: Service) {
    ...
    console.debug(`Unregistered services ${service.name}, version ${service.version} at ${service.ip}:${service.port}`);
    return key;
  };
  ...
  private cleanup() {
    ...
    Object.keys(this.services).forEach((key) => {
      if (this.services[key].timestamp + this.timeout < now) {
        delete this.services[key];
        console.debug(`Removing service ${key}`);
      }
    });
  };
};
